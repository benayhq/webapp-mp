...
import { AtModal, AtModalHeader, AtModalContent, AtModalAction,AtRate } from "taro-ui";
   ...
   async loadData(){
        ...
        //   this.props.dispatchActiveInfo(payload).then(res=>{
            ...
            if(reuslt.content && reuslt.content.commentVo && reuslt.content.commentVo.location.length>0){
                reuslt.content.commentVo.location.map((item)=>{
            ...
            if(reuslt.content &&  reuslt.content.docInfo && reuslt.content.docInfo.length > 0){
                reuslt.content.docInfo.map((item)=>{
        ...
        if(!loaded){
            ...
            style={{ height }}>
                ...
                <View className="mp-activedetail__price">
                    ...
                    <View>
                        <Text className="mp-activedetail__address"> {data && data.areaCode}</Text>
                ...
                <View className="mp-activedetail__person">
                    ...
                    <View className="mp-activedetail__content">
                        ...
                        <View className="mp-activedetail__content-score">
                            <View><AtRate value={data.commentScore} /></View>
                            <View> {data.commentScore}分（{ data &&  data.commentPeople !=null ? data.commentPeople:0}人评）</View>
                        </View>
                        <View>
                        服务区域: {data && data.areaCode}
                        </View>
                    </View>
                </View>
                {
                    data.activityBatchVos && data.activityBatchVos.length > 0 && (
                ...
                <View className="mp-activedetail__comment">
                    <View className="mp-activedetail__comment-title">
                        ...
                        <Text style="width:150px; height:35px; line-height:35px;left:-7px; top:-5px; position:relative;display:inline-block;">评价 ({data.commentSize === null ? 0 : data.commentSize})</Text>
                    </View>
                    {
                ...
                <View className="mp-activedetail__footer">
                    ...
                    <View className={data.status === 'CLOSE' ? 'mp-activedetail__action__footer_disable':'mp-activedetail__action__footer'} onClick={this.openCategoryDialog.bind(this,data.status)}>
                         {data.status === 'CLOSE' ? '拼团已结束' : '参与拼团' }
                    </View>
                </View>
            ...
            <AtModal isOpened={showOrderDialog}>
                <AtModalContent>
                    <TuanList dialog={this.openCategoryDialog.bind(this)} list={data.activityBatchVos}/>
                </AtModalContent>
