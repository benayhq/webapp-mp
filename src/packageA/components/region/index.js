...
export default class Region extends Component {
    ...
    async initData(){
        ...
        console.log('initData response',response.address)
        this.setState({
            region:response.address === "" ? "请选择省市区" : response.address,
            value:[6, 0, 0]
        });
    }
    ...
    onChange = (e) => {
        ...
        let  regionStr = regionTemp,regionArr = regionStr.split('/'),cacheValue = "";
        if(regionArr[0]==="全部"){
            cacheValue = "";
        }
        else{
